...
# FOR THE FILE EMOTIONAL MOTIVATIONAL FIXED MOTIVATIONAL
####################################
...
# Files to read
files = ["motivations_tables.csv", "motivations_tables_real.csv"]

# Path were data is saved
save_path = "/home/marcosm/Results/plots/motivations/"
saved_files_user = ["motivational_fixed_user.pdf", "motivational_fixed_user_real.pdf"]
saved_files_robot = ["motivational_fixed_robot.pdf", "motivational_fixed_robot_real.pdf"]
...
for fi, f in enumerate(files):
	...
	if len(user_profiles) > 4:
		...
		ax.set_xticklabels([])
		ax.axis('off')
		ax.legend(patches, mot, fancybox=True, loc='center', prop={'size': 25})
		# Save figure
		plt.savefig(save_path+saved_files_user[fi])
	else:
		lgd = fig.legend(patches, mot, bbox_to_anchor=(0.27,0.92,0.5,0.2), fancybox=True, loc='center', prop={'size': 25}, ncol=4, mode="expand")
		# Save figure
