...
# Files to read
files = ["comfort_simulation_fixed.csv", "comfort_real_fixed.csv"]
...
saved_filcomfort_user = ["comfort_fixed_user.pdf", "comfort_fixed_user_real.pdf"]
saved_filcomfort_robot = ["comfort_fixed_robot.pdf", "comfort_fixed_robot_real.pdf"]
...
for fi, f in enumerate(files):
	...
	colors = {"nD": "blue", "MnD": "red", "NR": "magenta", "MD": "lime", "FD": "yellow"}
	# Set labels
	...
	for i, up in enumerate(user_profiles):
		...
		ax.set_ylim([0, 100])
		if i == 0 or i == 4:
			ax.set_ylabel("Physiological stability", fontsize=title_fontsizes[fi]+2, weight="bold")
			ax.set_yticklabels(np.arange(0, 101, 20), fontsize=title_fontsizes[fi])
		else:
			ax.set_yticklabels([])
	...
	if len(user_profiles) > 4:
		...
		lgd = fig.legend(patches, robot_profiles, bbox_to_anchor=(0.27,0.92,0.5,0.2), fancybox=True, loc='center', prop={'size': 30}, ncol=4, mode="expand")
		# Save figure
		plt.savefig(save_path+saved_filcomfort_user[fi], bbox_extra_artists=(lgd,), bbox_inches='tight')
...
###########################################################
	...
	# Set labels
	labels = {"nD": "Non-Dependent\non the interaction", "MnD": "Mostly non-Dependent\non the interaction", "NR": "Equally Dependent\non internal and\nexternal factors", "MD": "Mostly Dependent\non the interaction", "FD": "Fully Dependent\non the interaction"}
	...
	if len(robot_profiles) > 4:
		...
		# Save figure
		plt.savefig(save_path+saved_filcomfort_robot[fi])
	else:
		lgd = fig.legend(patches, user_profiles, bbox_to_anchor=(0.27,0.92,0.5,0.2), fancybox=True, loc='center', prop={'size': 30}, ncol=4, mode="expand")
		# Save figure
		plt.savefig(save_path+saved_filcomfort_robot[fi], bbox_extra_artists=(lgd,), bbox_inches='tight')